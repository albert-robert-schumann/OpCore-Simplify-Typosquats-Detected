...
<p> My own repository is this: https://github.com/albert-robert-schumann/OpCore-Simplify-FixVulnerabilities and it has fixed vulnerabilities like outdated Google Chrome 131 user agent string. To see more what it has been fixed, read the readme in that repository. This repository doesn't have malware. I can't recommend the official repository because it's vulnerable. The maintainer doesn't fix the exploit unless it's actively exploited when it's too late or it has a proof of concept exploit - not every exploit can have a proof of concept. To me, it happened once that I pushed an issue for a 0 day exploit that allowed kext redirection, the maintainer rejected to fix it until he understood hackers weaponized it in both the typosquatting repos that I'll talk about and fixed it when attackers have already forked OpCore-Simplify for malicious purposes. If I was the maintainer of the official project, I'd have a look of the PRs that fix found exploits by users immediately and I wouldn't wait until the exploit is weaponized by black hats or for a proof of concept. And another time (fortunately not yet weaponized by hackers) is I pushed 5 PRs for 5 vulnerabilities of which 3 are related to outdated user agent strings, 1 of them saves passwords in plain text and another one saves MAC addresses in plain text and the maintainer unfortunately closed the PRs without fixing those exploits. It's extremely dangerous a project that operates with kernels not to fix those exploits unless proof of concept is available or it's actively weaponized. In fact exposing WiFi passwords in logs allows black hats to gain unauthorized WiFi access, which is extremely dangerous. In fact the maintainer of all exploits that I reported only fixed the ones that have proof of concepts - of which most of them could be weaponized even by non-experienced hackers.</p>
<p> When I reported the exploit where attackers could redirect downloads to malicious servers, he answered this: </p>
<img width="1552" height="915" alt="Bildschirmfoto 2025-11-28 um 23 17 27" src="https://github.com/user-attachments/assets/16305df0-1254-44f0-ae09-8a52f49e13e9" />
<img width="1552" height="915" alt="Bildschirmfoto 2025-11-28 um 23 17 38" src="https://github.com/user-attachments/assets/b26d6158-46c3-46b9-b9e4-46c886ddac52" />
<img width="1552" height="915" alt="Bildschirmfoto 2025-11-28 um 23 17 44" src="https://github.com/user-attachments/assets/c9097189-6341-4e11-a144-e1cd069c1b47" />
<img width="1552" height="915" alt="Bildschirmfoto 2025-11-28 um 23 17 47" src="https://github.com/user-attachments/assets/fb31593f-e616-4e50-a2ae-0c207c4527eb" />
<img width="1552" height="915" alt="Bildschirmfoto 2025-11-28 um 23 17 50" src="https://github.com/user-attachments/assets/e075a5be-d479-42a9-9d3f-dd55a4fd6ba1" />
<img width="1552" height="915" alt="Bildschirmfoto 2025-11-28 um 23 18 15" src="https://github.com/user-attachments/assets/41059ca8-57e1-4d42-8903-80f76d563c98" />
<img width="1552" height="915" alt="Bildschirmfoto 2025-11-28 um 23 18 18" src="https://github.com/user-attachments/assets/277bcfe8-8af1-4e07-a276-198f7ede87e2" />
...
This project exploits a vulnerability in older OpCore-Simplify versions to download kexts from whatever source the attacker wants to to fully compromise your systems. That vulnerability is now patched recently, but by the time it got patched attackers have already forked this project and abused the kext redirection vulnerability. An attacker like this can gain privileged kernel access at ring 0 to avoid antivirus detection. This is the highest privilege level the attacker can gain inside the kernel. Another red flag is that the attacker has written either generic commits or commits in Viatnamese. In most legitimate cases, if the owner of the repo targets international audience write commits in English, not Viatnamese, nor Chinese - this is obfuscation technique so it's much harder for non-Viatnamese speakers to understand what exactly is going on. Let's now go into the kext_data.py and see what's going on:
